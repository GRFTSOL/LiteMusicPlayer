// MLProtocol.cpp: implementation of the CMLProtocol class.
//
//////////////////////////////////////////////////////////////////////
...
int CMLProtocol::readMsg(MLMsgType &msgType, string *pMsgBuff)
{
    ...
    httpProtocol->setCacheData(pMsgBuff->data(), pMsgBuff->size());
...
int CMLProtocol::readOldXmlMsg(MLMsgType &msgType, string *pMsgBuff)
{
    ...
    {
        int nRet = m_pNetFile->readExactly(*pMsgBuff, ML_MSG_XML_LEN + ML_MSG_XML_DATA_LEN - pMsgBuff->size());
        if (nRet != ERR_OK)
            return nRet;
    }
    ...
    int nRet = m_pNetFile->readExactly(*pMsgBuff, nDataLength - pMsgBuff->size());
    if (nRet != ERR_OK)
        return nRet;
...
int CMLProtocol::writeOldXmlMsg(string *pMsgBuff)
{
    ...
    snprintf(szHeader, CountOf(szHeader), ML_MSG_XML_FMT, pMsgBuff->size());

    nRet = m_pNetFile->write(szHeader, ML_MSG_XML_LEN + ML_MSG_XML_DATA_LEN, &nWrite);
    if (nRet != ERR_OK)
        return nRet;

    nRet = m_pNetFile->write(pMsgBuff->data(), pMsgBuff->size(), &nWrite);
    if (nRet != ERR_OK)
        return nRet;
