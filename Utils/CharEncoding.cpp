...
CHAR_ENCODING getCharEncodingID(const char *szEncoding) {
    ...
    for (int i = getCharEncodingCount() - 1; i >= 0; i--) {
        ENCODING_CODEPAGE &ec = g_encodingCodePages[i];
...
CHAR_ENCODING detectFileEncoding(const void *lpData, int length, int &bomSize)
{
...
cstr_t getFileEncodingBom(CHAR_ENCODING encoding)
{
    switch (encoding)
    {
...
string strToUtf8ByBom(const char *data, size_t size) {
    int bomSize = 0;
    CHAR_ENCODING encoding = detectFileEncoding(data, size, bomSize);
    data += bomSize;
    size -= bomSize;
