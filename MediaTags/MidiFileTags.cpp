...
public:
    ...
    int getLyrics(RawLyrics &lyricsLines);
...
int CMidiTag::listLyrics(VecStrings &vLyrNames) {
    if (hasLyrics()) {
        vLyrNames.push_back(SZ_SONG_SOLE_EMBEDDED_LYRICS);
        return ERR_OK;
    }
    return ERR_NOT_FOUND;
}

int CMidiTag::getLyrics(RawLyrics &lyricsLines) {
        ...
        if (eventCode >= MET_NOTE_OFF && eventCode <= MET_CONTROL_CHANGE) {
            ...
            if (metaEvent == MAT_TEMPO) {
                ...
                if (bNewLine || line == nullptr) {
                    lyricsLines.push_back(LyricsLine((int)startTime, -1));
                    line = &lyricsLines.lines().back();
                }
                line->appendPiece((int)startTime, -1, str);
...
int CMidiTag::readTrackTrunk(TrackTrunk &trunk) {
    ...
    while (m_file.getPos() < long(trunk.offset + trunk.sizeOfData)) {
        ...
        if (eventCode >= MET_NOTE_OFF && eventCode <= MET_CONTROL_CHANGE) {
            ...
            // m_file.seek(len, SEEK_CUR);
            buf.resize(len);
            nRet = m_file.readCount(buf.data(), len);
            if (nRet != ERR_OK) {
                return nRet;
            }
...
string MidiFileTags::getFileKind(const StringView &fileExt) {
    return "Musical Instrument Digital Interface File (midi)";
}

int MidiFileTags::getTags(cstr_t fileName, BasicMediaTags &tagsOut, ExtendedMediaInfo &extendedInfoOut) {
    return ERR_NOT_FOUND;
}

VecStrings MidiFileTags::enumLyrics(cstr_t fileName) {
    return { SZ_SONG_SOLE_EMBEDDED_LYRICS };
}

int MidiFileTags::getLyrics(cstr_t fileName, cstr_t lyricsUrl, bool isUseSpecifiedEncoding, CharEncodingType encodingSpecified, RawLyrics &rawLyricsOut) {
    if (!StringView(lyricsUrl).equal(SZ_SONG_SOLE_EMBEDDED_LYRICS)) {
        return ERR_NOT_FOUND;
    }

    CMidiTag tag;
    int ret = tag.open(fileName);
    if (ret != ERR_OK) {
        return ret;
    }

    return tag.getLyrics(rawLyricsOut);
}
