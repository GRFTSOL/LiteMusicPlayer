...
// append text included 0x0 and bom(for ucs2) to buffer.
void appendStrByEncodingAndBom(string &buff, cstr_t str, ID3v2EncType encType, CHAR_ENCODING encoding = ED_SYSDEF);

// return the size of bytes used from buffer data
int copyAnsiStr(string &str, const char *data, int nLen);

// return the size of bytes used from buffer data
int copyStrByEncodingAndBom(string &str, ID3v2EncType encType, const char *data, int len, CHAR_ENCODING encoding = ED_SYSDEF);
