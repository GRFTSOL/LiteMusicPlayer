...
int fileAppendData(FILE *m_fp, long nAppend, int nMask) {
    char buff[4096];
    ...
    for (long i = nAppend; i > 0; i -= sizeof(buff)) {
        long nLen;
        if (i >= sizeof(buff)) {
            nLen = sizeof(buff);
        } else {
        ...
        if (fwrite(buff, 1, nLen, m_fp) != nLen) {
            // write error!
            filetruncate(m_fp, (int)orgLength);
...
int fileMoveEndData(FILE *m_fp, long nOffset, long nNewOffset) {
    ...
    long nLen, nPos, nNewAppend;

    if (fseek(m_fp, 0, SEEK_END) != 0) {
        return ERR_SEEK_FILE;
    }

    auto nFileLen = ftell(m_fp);
    ...
    if (nNewOffset == nOffset) {
        ...
        auto nRet = fileAppendData(m_fp, nNewOffset - nOffset);
        if (nRet != ERR_OK) {
            return nRet;
        }

        nNewAppend = nNewOffset - nOffset;
        auto nLastEndPos = nFileLen;
