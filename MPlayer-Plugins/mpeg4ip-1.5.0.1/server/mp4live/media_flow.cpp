...
void CAVMediaFlow::stop(void)
{
	...
	while (mc != NULL) {
	  //debug_message("stopping video profile %s", mc->GetFile());
	  if (m_videoSource != NULL) {
	...
	while (mc != NULL) {
	  // debug_message("stopping audio profile %s", mc->GetFile());
	  mc->StopThread();
	...
	while (mc != NULL) {
	   debug_message("stopping text profile %s", mc->GetFile());
	  mc->StopThread();
	  debug_message("thread stopped");
	  mc->StopSinks();
	  debug_message("stopping sinks");
						       ...
						       bool create)
{
  ...
  while (ve_ptr != NULL) {
    if (strcmp(vp_name, ve_ptr->GetFile()) == 0) {
      return ve_ptr;
    }
...
CAudioEncoder *CAVMediaFlow::FindOrCreateAudioEncoder (CAudioProfile *ap)
{
  ...
  while (ae_ptr != NULL) {
    if (strcmp(ap_name, ae_ptr->GetFile()) == 0) {
      return ae_ptr;
    }
...
CTextEncoder *CAVMediaFlow::FindOrCreateTextEncoder (CTextProfile *tp)
{
  ...
  while (te_ptr != NULL) {
    if (strcmp(tp_name, te_ptr->GetFile()) == 0) {
      return te_ptr;
    }
