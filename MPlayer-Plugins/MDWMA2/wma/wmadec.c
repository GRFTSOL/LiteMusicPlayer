...
#define IRINT(_v, _s)	(((_v) + (1 << ((_s) - 1))) >> (_s))
#endif

#  define rint(x)   (floor((x)+0.5f)) 

__inline long int llrintf(float x)
{
    return (int)(rint(x));
}

#ifdef NO_FPTOFX_ASSERT
#define FPTOFXS(_fp, _s)	((int32_t)(lrintf((_fp) * (int32_t)((1L<<(_s))-1))))
#define NFPTOFXS(_fp, _s)	((int32_t)(lrintf((_fp) * (int32_t)(-(1L<<(_s))))))
#define FPTOFXU(_fp, _s)	((uint32_t)(llrintf((_fp) * (uint32_t)((1L<<(_s))-1))))
#else
#define FPTOFXS(_fp, _s) ({					\
    float __fp = (_fp);						\
    assert((__fp >= (float)-(1<<(31-(_s)))) &&			\
	   (__fp <= (float)(1<<(31-(_s)))));			\
    ((int32_t)(lrintf(__fp * (int32_t)((1L<<(_s))-1))));	\
})
#define NFPTOFXS(_fp, _s) ({					\
    float __fp = (_fp);						\
    assert((__fp >= (float)-(1<<(31-(_s)))) &&			\
	   (__fp <= (float)(1<<(31-(_s)))));			\
    ((int32_t)(lrintf(__fp * (int32_t)(-(1L<<(_s))))));	\
})
#define FPTOFXU(_fp, _s) ({					\
    float __fp = (_fp);						\
    assert((__fp >= 0.f) && (__fp <= (float)(1<<(32-(_s)))));	\
    ((uint32_t)(llrintf(__fp * (uint32_t)((1L<<(_s))-1))));	\
})
#endif
...
typedef struct WMADecodeContext {
    ...
    uint32_t iexponents[MAX_CHANNELS][BLOCK_MAX_SIZE]; /* << 19 */
...
static int wma_decode_init(AVCodecContext * avctx)
{
    ...
    if (s->use_noise_coding) {
...
#else
        {
            ...
            float norm;
            seed = 1;
            norm = (512.0 / (float)(1L << 31)) * sqrt(3) * s->noise_mult;
            for(i=0;i<NOISE_TAB_SIZE;i++) {
                seed = seed * 314159 + 1;
...
/* compute x^-0.25 with an exponent and mantissa table. We use linear
   ...
   bits of precision). */
__inline float pow_m1_4(WMADecodeContext *s, float x)
{
    union {
        float f;
        unsigned int v;
...
// XXX: FIXME!! there's a bug somewhere which makes this mandatory under altivec
#ifdef HAVE_ALTIVEC
    volatile int nb_coefs[MAX_CHANNELS];
#else
    ...
    for(ch = 0; ch < s->nb_channels; ch++) {
        if (s->channel_coded[ch]) {
	    int32_t output[BLOCK_MAX_SIZE * 2];
            int32_t *ptr;
            int i, n4, index, n;
