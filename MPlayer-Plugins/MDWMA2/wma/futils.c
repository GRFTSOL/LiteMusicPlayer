...
{
    uint8_t *data = (uint8_t *)malloc(size + FF_INPUT_BUFFER_PADDING_SIZE);
    if (!data)
...
int av_dup_packet(AVPacket *pkt)
{
    if (pkt->destruct != av_destruct_packet) {
        ...
        /* we duplicate the packet and don't forget to put the padding
           again */
        data = (uint8_t *)malloc(pkt->size + FF_INPUT_BUFFER_PADDING_SIZE);
        if (!data) {
            return AVERROR_NOMEM;
        }
...
/* convert the packet time stamp units and handle wrapping. The
   ...
   rounding errors. */
__inline int64_t convert_timestamp_units(AVFormatContext *s,
                  ...
                  int (*alloc_cb)(void *, AVImageInfo *info), void *opaque)
{
    ...
    if (!fmt) {
        ...
        pos = url_ftell(pb);
        pd->buf_size = get_buffer(pb, (unsigned char *)buf, PROBE_BUF_SIZE);
