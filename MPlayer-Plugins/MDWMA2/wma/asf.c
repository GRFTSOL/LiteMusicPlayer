...
const CodecTag ffmpeg_codec_wav_tags[] = {
...
{
    int id;
    id = codec_get_id(ffmpeg_codec_wav_tags, tag);
    if (id <= 0)
        return id;
...
void get_wav_header(ByteIOContext *pb, AVCodecContext *codec, int size)
{
    ...
    codec->codec_id = (enum CodecID)wav_codec_get_id(id, codec->bits_per_sample);
...
static void print_guid(const GUID *g)
{
    ...
    PRINT_IF_GUID(g, asf_header);
        ...
        printf("(GUID: unknown) ");
    printf("0x%08x, 0x%04x, 0x%04x, {", g->Data1, g->Data2, g->Data3);
    for(i=0;i<8;i++)
        printf(" 0x%02x,", g->Data4[i]);
    printf("}\n");
}
...
static void get_guid(ByteIOContext *s, GUID *g)
{
    ...
    g->Data3 = get_le16(s);
    for(i=0;i<8;i++)
        g->Data4[i] = get_byte(s);
}

static void get_str16_nolen(ByteIOContext *pb, int len, TCHAR *buf, int buf_size)
{
    int		c, lenz;
    TCHAR	*q;
	int		bEraseOneByte;
...
static int asf_probe(AVProbeData *pd)
{
    ...
    if (pd->buf_size <= 32)
        return 0;
    p = pd->buf;
    g.Data1 = p[0] | (p[1] << 8) | (p[2] << 16) | (p[3] << 24);
    p += 4;
    g.Data2 = p[0] | (p[1] << 8);
    p += 2;
    g.Data3 = p[0] | (p[1] << 8);
...
static int asf_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    for(;;) {
	    ...
	    st->id = get_le16(pb) & 0x7f; /* stream id */
            ...
            get_le32(pb);
	    st->codec.codec_type = (enum CodecType)type;
		...
		if (asf_st->ds_span > 1) {
            ...
            len4 = get_le16(pb);
            len5 = get_le16(pb);
            get_str16_nolen(pb, len1, s->title, CountOf(s->title));
            get_str16_nolen(pb, len2, s->author, CountOf(s->author));
            get_str16_nolen(pb, len3, s->copyright, CountOf(s->copyright));
            get_str16_nolen(pb, len4, s->comment, CountOf(s->comment));
	    url_fskip(pb, len5);
                ...
                for(i=0;i<desc_count;i++)
                {
                        ...
                        get_str16_nolen(pb, name_len, name, CountOf(name));
                        value_type = get_le16(pb);
                        value_len = get_le16(pb);
                        if ((value_type == 0) || (value_type == 1)) // unicode or byte
                        {
                            get_str16_nolen(pb, value_len, value, CountOf(value));
                            if (wcscmp(name,_T("WM/AlbumTitle"))==0) { wcscpy(s->album, value); }
                            else if (wcscmp(name,_T("WM/Genre"))==0) { wcscpy(s->genre, value); }
                            else if (wcscmp(name,_T("WM/Year"))==0) s->year = _wtoi(value);
				}
                        if ((value_type >= 2) && (value_type <= 5)) // boolean or DWORD or QWORD or WORD
                        {
                            ...
                            if (wcscmp(name,_T("WM/Track"))==0) s->track = value_num + 1;
                            else if (wcscmp(name,_T("WM/TrackNumber"))==0) s->track = value_num;
                        }
                }
        } else if (url_feof(pb)) {
...
//s->streams[asf->stream_index]->codec.codec_type == CODEC_TYPE_AUDIO, asf->packet_obj_size);
	...
	if (asf_st->frag_offset == asf_st->pkt.size) {
	    ...
	    if (asf_st->ds_span > 1) {
		/* packet descrambling */
		char* newdata = (char *)malloc(asf_st->pkt.size);
		if (newdata) {
		    int offset = 0;
...
static int64_t asf_read_pts(AVFormatContext *s, int64_t *ppos, int stream_index)
{
    ...
    int64_t pos= *ppos;
    int i;
    int64_t *start_pos = malloc(sizeof(int64_t) * s->nb_streams);
