...
{
	if (a <= 21) return 0;
	if (a <= 50) return 0.5842*pow((float)a-21, (float)0.4)+0.07886*(a-21);
	return 0.1102*(a-8.7);
}
...
void equ_init(int wb)
{
	...
	if (fspectrum != NULL) free(fspectrum);
	...
	fspectrum  = (REAL *)malloc(sizeof(REAL)*tabsize);
	...
	while(nbufsamples+nsamples >= winlen)
	{
		...
		for(ch=0;ch<nch;ch++)
		{
			...
			if (enable) {
				...
				fspectrum[0] = sqrt(fsamples[0] * fsamples[0] + fsamples[1] * fsamples[1]);
				...
				for(i=1;i<tabsize/2;i++)
				{
					...
					fspectrum[i + ch * tabsize/2] = sqrt(re * re + im * im);
		...
		OnSpectrumData(fspectrum, tabsize / 2, nch);
	}
	...
	case 16:
		for(i=0;i<nsamples*nch;i++)
		{
			...
			if (dither) {
				...
				if (s < amin) s = (float)amin;
				if (amax < s) s = (float)amax;
				((short *)buf)[i+p*nch] = RINT(s);
			}
		}
		break;
